...
public class Interpreter {
    ...
    private enum Type {
        ...
        EQUATION(10), PLUS(30), MINUS(30), MULTIPLE(50), DIVIDE(50), MOD(50), LEFT_BRACKET(-10), RIGHT_BRACKET(-10),
...
//        typeMap.put("=", Type.EQUATION);
    ...
    private static class Evaluation {
        enum OP {
            PLUS, MINUS, MULTIPLE, DIVIDE, MOD, READ_VARIABLE, SAVE_VARIABLE, PUSH_STACK, CALL_FUNCTION
        }
        ...
        while (iterator.hasNext()) {
            ...
            switch (word.type) {
                ...
                case LEFT_BRACKET:
                    ...
                    break;
                case RIGHT_BRACKET:
                    while (op.peek().type != Type.LEFT_BRACKET)
                        instructors.add(Evaluation.Instructor.valueOf(op.pop()));
                    op.pop();
                    break;
                default:
    ...
    private static Deque<Word> markingType(Deque<String> tokens) {
        ...
        for (String token : tokens) {
            try {
                ...
                if (typeMap.containsKey(token)) words.add(new Word(typeMap.get(token), token));
                else if ("=".equals(token)) {
                    if (words.peekLast().type != Type.VARIABLE) throw new RuntimeException("Const can't set value.");
